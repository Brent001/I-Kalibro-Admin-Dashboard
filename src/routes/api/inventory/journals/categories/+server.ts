// src/routes/api/journals/categories/+server.ts

import { json, error } from '@sveltejs/kit';
import type { RequestHandler } from './$types.js';
import jwt from 'jsonwebtoken';
import { db } from '$lib/server/db/index.js';
import { tbl_staff, tbl_admin, tbl_super_admin, tbl_category, tbl_journal, tbl_book, tbl_magazine, tbl_thesis } from '$lib/server/db/schema/schema.js';
...
export const GET: RequestHandler = async ({ request }) => {
  try {
    ...
    const itemType = (url.searchParams.get('itemType') || 'journal').toString();
...
export const POST: RequestHandler = async ({ request }) => {
  try {
    ...
    const itemType = (body.itemType || 'journal').toString();
    ...
    // Check for duplicates case-insensitively within same item type
...
export const PUT: RequestHandler = async ({ request }) => {
  try {
    ...
    // Check for duplicates case-insensitively (excluding current category) within same item type

...
export const load: PageServerLoad = async ({ cookies, url }) => {
    ...
    try {
        if (await isSessionRevoked(token)) {
            console.warn('[members] Session revoked');
            clearAuthCookies(cookies);
            throw redirect(302, '/');
        }
    } catch (err) {
        ...
        console.warn('[members] isSessionRevoked check failed (non-fatal):', (err as any)?.message);
    }
    ...
    if (!userId) {
        console.warn('[members] Token missing user ID');
        clearAuthCookies(cookies);
        throw redirect(302, '/');
    }

...
export const load: PageServerLoad = async ({ cookies, url }) => {
    ...
    try {
        if (await isSessionRevoked(token)) {
            console.warn('[security_logs] Session revoked');
            clearAuthCookies(cookies);
            throw redirect(302, '/');
        }
    } catch (err) {
        ...
        console.warn('[security_logs] isSessionRevoked check failed (non-fatal):', (err as any)?.message);
    }
    ...
    if (!userId) {
        console.warn('[security_logs] Token missing user ID');
        clearAuthCookies(cookies);
        throw redirect(302, '/');
    }
    ...
    if (requestedUid) {
        ...
        if (userType !== 'super_admin' && requestedUid !== shortUid) {

<script lang="ts">
  ...
  async function fetchCategories() {
    if (!browser) return;
    try {
      const response = await fetch('/api/inventory/journals/categories', { credentials: 'include' });
      if (response.ok) {
  ...
  async function fetchStats() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data?.data) {
          stats = {
            totalBooks: data.data.totalJournals ?? 0,
            totalJournals: data.data.totalJournals ?? 0,
            availableCopies: data.data.availableCopies ?? 0,
            borrowedBooks: data.data.borrowedJournals ?? 0,
            borrowedJournals: data.data.borrowedJournals ?? 0,
            ...
            availability: data.data.availability ?? 0
          };
        } else {
          stats = {
            totalBooks: 0,

    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
      <div class="bg-white p-6 rounded-lg shadow-sm border border-gray-200 flex items-center justify-between">
        <div>
          ...
          <p class="text-sm text-slate-600 mt-1">+125 this month</p>
        </div>
        <div class="p-3 bg-slate-50 rounded-lg">
          <svg class="h-8 w-8 text-slate-700" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
      ...
      <div class="bg-white p-6 rounded-lg shadow-sm border border-gray-200 flex items-center justify-between">
        <div>
          ...
          <p class="text-sm text-slate-600 mt-1">+48 this month</p>
        </div>
        <div class="p-3 bg-slate-50 rounded-lg">
          <svg class="h-8 w-8 text-slate-700" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
          ...
          {#each recentActivity as activity}
            <div class="p-4 border-b border-gray-100 last:border-b-0 hover:bg-gray-50 transition-colors duration-150">
              <div class="flex items-start space-x-3">
                <div class="w-2 h-2 bg-slate-900 rounded-full mt-2"></div>
                <div class="flex-1">
                  <p class="text-sm font-medium text-gray-900">{activity.action}</p>
                  {#if activity.book}
                    <p class="text-sm text-gray-600">{activity.book}</p>
                  {/if}
                  <div class="flex items-center justify-between mt-1">
                    <span class="text-sm text-slate-700 font-medium">{activity.member}</span>
          ...
          {#each overdueBooks as book}
            <div class="p-4 border-b border-gray-100 last:border-b-0 hover:bg-gray-50 transition-colors duration-150">
              <div class="flex items-start space-x-3">
                ...
                <div class="flex-1">
                  <p class="text-sm font-medium text-gray-900">{book.book}</p>
                  <p class="text-sm text-gray-600">{book.member}</p>
                  <div class="flex items-center justify-between mt-2">
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800">
                      {book.daysOverdue} days overdue
                    </span>
                    <span class="text-xs text-gray-500">Due: {book.dueDate}</span>
                  </div>
                </div>
              </div>
            </div>

...
export const load: PageServerLoad = async ({ cookies, url }) => {
    ...
    try {
        if (await isSessionRevoked(token)) {
            console.warn('[reports/visits] Session revoked');
            clearAuthCookies(cookies);
            throw redirect(302, '/');
        }
    } catch (err) {
        ...
        console.warn('[reports/visits] isSessionRevoked check failed (non-fatal):', (err as any)?.message);
    }
    ...
    if (!userId) {
        console.warn('[reports/visits] Token missing user ID');
        clearAuthCookies(cookies);
        throw redirect(302, '/');
    }

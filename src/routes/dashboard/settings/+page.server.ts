...
export const load: PageServerLoad = async ({ cookies, url }) => {
    ...
    try {
        if (await isSessionRevoked(token)) {
            console.warn('[settings] Session revoked');
            clearAuthCookies(cookies);
            throw redirect(302, '/');
        }
    } catch (err) {
        ...
        console.warn('[settings] isSessionRevoked check failed (non-fatal):', (err as any)?.message);
    }
    ...
    if (!userId) {
        console.warn('[settings] Token missing user ID');
        clearAuthCookies(cookies);
        throw redirect(302, '/');
    }

...
export const load: PageServerLoad = async ({ cookies }) => {
    ...
    try {
        if (await isSessionRevoked(token)) {
            console.warn('[staff] Session revoked');
            clearAuthCookies(cookies);
            throw redirect(302, '/');
        }
    } catch (err) {
        ...
        console.warn('[staff] isSessionRevoked check failed (non-fatal):', (err as any)?.message);
    }
    ...
    if (!userId) {
        console.warn('[staff] Token missing user ID');
        clearAuthCookies(cookies);
        throw redirect(302, '/');
    }
